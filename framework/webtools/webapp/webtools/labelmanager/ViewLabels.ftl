  ...
  <table class="basic-table hover-bar" cellspacing="3">
    <tr class="header-row">
      ...
      <#list localesFound as localeFound>
        ...
        <#if showLocale == true>
          <#assign locale = Static["com.hanlin.fadp.base.util.UtilMisc"].parseLocale(localeFound)!/>
      ...
      <#list labelsList as labelList>
        ...
        <#if showLabel == true>
          <tr <#if rowNum == "1">class="alternate-row"</#if>>
            <td>${rowNumber}</td>
            <td><a href="<@fadpUrl>UpdateLabel?sourceKey=${labelKey}&amp;sourceFileName=${label.fileName}&amp;sourceKeyComment=${label.labelKeyComment!}</@fadpUrl>" <#if previousKey == labelKey>class="submenutext"</#if>>${label.labelKey}</a></td>
            <td>${label.fileName}</td>
            <td><a href="<@fadpUrl>ViewReferences?sourceKey=${labelKey}&amp;labelFileName=${label.fileName}</@fadpUrl>">${uiLabelMap.WebtoolsLabelManagerReferences}</a></td>
            <#list localesFound as localeFound>
