  ...
  <#if showWsdl?? && showWsdl = true>
    <div class="screenlet">
      ...
      <div class="screenlet-body" align="center">
        ...
        <br />
        <a href='<@fadpUrl>${url}?sel_service_name=${selectedServiceMap.serviceName}</@fadpUrl>' class='smallSubmit'>${uiLabelMap.CommonBack}</a>
      </div>
    </div>
  <#else>
    <div class="screenlet">
      <div class="screenlet-title-bar">
        <ul>
          ...
          <li><a href='<@fadpUrl>${url}</@fadpUrl>'>${uiLabelMap.CommonListAll}</a></li>
          <li><a href='<@fadpUrl>/scheduleJob?SERVICE_NAME=${selectedServiceMap.serviceName}</@fadpUrl>'>${uiLabelMap.WebtoolsSchedule}</a></li>
          <li><a href='<@fadpUrl>/setSyncServiceParameters?SERVICE_NAME=${selectedServiceMap.serviceName}&amp;POOL_NAME=pool&amp;_RUN_SYNC_=Y</@fadpUrl>'>${uiLabelMap.PageTitleRunService}</a></li>
        </ul>
        <br class="clear"/>
      </div>
      <div class="screenlet-body">
        <table class="basic-table" cellspacing='0'>
          <tr>
            ...
            <td><a href='<@fadpUrl>${url}?constraint=engine_name@${selectedServiceMap.engineName}</@fadpUrl>'>${selectedServiceMap.engineName}</a></td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td>${selectedServiceMap.export}<#if selectedServiceMap.exportBool = "true">&nbsp;(<a href='<@fadpUrl>${url}?sel_service_name=${selectedServiceMap.serviceName}&amp;show_wsdl=true</@fadpUrl>'>${uiLabelMap.WebtoolsShowShowWSDL}</a>)</#if></td>
            <td class="label">${uiLabelMap.WebtoolsLocation}</td>
            <td><a href='<@fadpUrl>${url}?constraint=location@${selectedServiceMap.location}</@fadpUrl>'>${selectedServiceMap.location}</a></td>
          </tr>
          <tr>
            <td class="label">${uiLabelMap.WebtoolsDefinitionLocation}</td>
            <td><a href='<@fadpUrl>${url}?constraint=definitionLocation@${selectedServiceMap.definitionLocation}</@fadpUrl>'>${selectedServiceMap.definitionLocation}</a></td>
            <td class="label">${uiLabelMap.WebtoolsDefaultEntityName}</td>
            <td><a href='<@fadpUrl>${url}?constraint=default_entity_name@${selectedServiceMap.defaultEntityName}</@fadpUrl>'>${selectedServiceMap.defaultEntityName}</a></td>
          </tr>
          <tr>
            <td class="label">${uiLabelMap.WebtoolsArtifactInfo}</td>
            <td><a href='<@fadpUrl>ArtifactInfo?name=${selectedServiceMap.serviceName}&amp;type=service</@fadpUrl>'>${uiLabelMap.WebtoolsArtifactInfo}</a></td>
      ...
      <div class="screenlet-body">
        ...
        <#elseif selectedServiceMap.implServices?has_content>
          <#list selectedServiceMap.implServices as implSrv>
            <a href='<@fadpUrl>${url}?sel_service_name=${implSrv.getService()}</@fadpUrl>'>${implSrv.getService()}</a><br />
          </#list>
        </#if>
      </div>
    </div>
              ...
              <#if ecaMap.actions?has_content>
                <td>
                  <#list ecaMap.actions as action>
                    <table class="basic-table" cellspacing='0'>
                      <tr>
                        <td colspan="2"><a href='<@fadpUrl>${url}?sel_service_name=${action.serviceName}</@fadpUrl>'>${action.serviceName?default(uiLabelMap.CommonNA)}</a></td>
                      </tr>
                      <tr>
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          <#if condition.conditionService?has_content>
                            <a href='<@fadpUrl>${url}?sel_service_name=${condition.conditionService}</@fadpUrl>'>${condition.conditionService?default(uiLabelMap.CommonNA)}</a>
                          <#else>
                ...
                <tr>
                  ...
                  <td>
                    <#if modelParam.entityName??>
                      <a href='<@fadpUrl>${url}?constraint=default_entity_name@${modelParam.entityName}</@fadpUrl>'>${modelParam.entityName!}</a>
                    </#if>
                  </td>
    ...
    <div class="screenlet-body">
      <form name="exportServiceEoModelBundle" method="post" action="<@fadpUrl>exportServiceEoModelBundle</@fadpUrl>" class="basic-form">
  ...
  <#if serviceNamesAlphaList?? && serviceNamesAlphaList?has_content>
      <div class="button-bar">
        ...
        <#list serviceNamesAlphaList as alpha>
          <a href='<@fadpUrl>${url}?constraint=alpha@${alpha}</@fadpUrl>'>${alpha}</a>
      ...
      <table class="basic-table hover-bar" cellspacing='0'>
        ...
        <#list servicesList as service>
          <tr<#if alt_row> class="alternate-row"</#if>>
            <td><a href='<@fadpUrl>${url}?sel_service_name=${service.serviceName}</@fadpUrl>'>${service.serviceName}</a></td>
            <td><a href='<@fadpUrl>${url}?constraint=engine_name@${service.engineName?default(uiLabelMap.CommonNA)}</@fadpUrl>'>${service.engineName}</a></td>
            <td><a href='<@fadpUrl>${url}?constraint=default_entity_name@${service.defaultEntityName?default(uiLabelMap.CommonNA)}</@fadpUrl>'>${service.defaultEntityName}</a></td>
            <td>${service.invoke}</td>
            <td><a href='<@fadpUrl>${url}?constraint=location@${service.location?default(uiLabelMap.CommonNA)}</@fadpUrl>'>${service.location}</a></td>
            <td><a href='<@fadpUrl>${url}?constraint=definitionLocation@${service.definitionLocation}</@fadpUrl>'>${service.definitionLocation}</a></td>
          </tr>
...
<#else>
  ...
  <a href='<@fadpUrl>${url}</@fadpUrl>' class="smallSubmit">${uiLabelMap.CommonListAll}</a>
</#if>
