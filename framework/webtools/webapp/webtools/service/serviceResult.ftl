...
<div class="screenlet">
  ...
  <div class="screenlet-body" align="center">
    <form method="post" action="<@fadpUrl>saveServiceResultsToSession</@fadpUrl>"
          ...
          <tr class="header-row">
            ...
            <#if serviceResultList?has_content>
              <#list serviceResultList as srl>
                <tr>
                  <#if srl.hasChild=="Y">
                    <td><a href="<@fadpUrl>/serviceResult?servicePath=</@fadpUrl><#if parameters.servicePath??>${parameters.servicePath}||</#if>${srl.key!}">${srl.key!}</a></td>
                  <#else>
                    <td>${srl.key!}</td>
                  </#if>
                    <td>${srl.value!}</td>
