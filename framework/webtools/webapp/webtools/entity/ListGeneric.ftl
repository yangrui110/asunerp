            ...
            <#if resultPartialList?has_content>
                ...
                <#list records as record>
                    <tr<#if alt_row> class="alternate-row"</#if>>
                        <td class="button-col">
                            <a href='<@fadpUrl>ViewGeneric?${record.findString}</@fadpUrl>'>${uiLabelMap.CommonView}</a>
                        <#if hasDeletePermission == 'Y'>
                            <a href='<@fadpUrl>UpdateGeneric?${record.findString}&amp;UPDATE_MODE=DELETE</@fadpUrl>'>${uiLabelMap.CommonDelete}</a>
                        </#if>
                        </td>
                        <#list fieldList as field>
                            <td>${Static["com.hanlin.fadp.base.util.UtilFormatOut"].makeString(record.fields.get(field.name))}</td>
