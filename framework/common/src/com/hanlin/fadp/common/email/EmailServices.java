 ...
 * under the License.
 *******************************************************************************/
package com.hanlin.fadp.common.email;
...
import com.hanlin.fadp.service.mail.MimeMessageWrapper;
import com.hanlin.fadp.webapp.view.ApacheFopWorker;
import com.hanlin.fadp.widget.renderer.fo.FoScreenRenderer;
...
public class EmailServices {
    ...
    public static Map<String, Object> sendMail(DispatchContext ctx, Map<String, ? extends Object> context) {
        ...
        String sendCc = (String) context.get("sendCc"); // Carbon Copy(抄送)(比如你要发邮件给你一个朋友，但是同时你又希望另一个
        //朋友可以看到你给第一个朋友的邮件内容，这样的话，你可以在寄的时候cc一下，相当于两个人都收到同份邮件，内容一样，就是在前头部分一个是send to(寄)
        //一个是cc（抄送） )
        String sendBcc = (String) context.get("sendBcc");// Blind CarbonCopy(暗抄送)
