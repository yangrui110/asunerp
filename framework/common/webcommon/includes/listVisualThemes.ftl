...
<div class="screenlet" style="margin: 1em;">
  <div class="screenlet-title-bar">
    <ul>
      ...
      <li><a href="<@fadpUrl>main</@fadpUrl>">${uiLabelMap.CommonDone}</a></li>
    </ul>
    <br class="clear"/>
  </div>
  <li class="h2" style="padding-top:1em">${uiLabelMap.CommonVisualThemeUsage}</li>
  <#if visualThemes?has_content>
    <#assign orderByList = Static["com.hanlin.fadp.base.util.UtilMisc"].toList("visualThemeId")/>
    <table cellspacing="0" class="basic-table">
      <#list visualThemes as visualTheme>
        <#assign screenshots = delegator.findByAnd("VisualThemeResource", Static["com.hanlin.fadp.base.util.UtilMisc"].toMap(
        ...
        <tr<#if visualTheme.visualThemeId == visualThemeId> class="selected"</#if>>
          <td>
            <form name="SetUserPreferences_${visualTheme.visualThemeId}" method="post" action="<@fadpUrl>setUserPreference</@fadpUrl>">
          ...
          <td>
            <#if screenshots?has_content>
              <#list screenshots as screenshot>
                <a id="single_image" href="<@fadpContentUrl>${screenshot.resourceValue}</@fadpContentUrl>"><img src="<@fadpContentUrl>${screenshot.resourceValue}</@fadpContentUrl>" width="150" alt=""/></a>
              </#list>
           <#else>
