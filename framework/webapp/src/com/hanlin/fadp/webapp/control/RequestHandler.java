...
import com.hanlin.fadp.webapp.event.EventFactory;
import com.hanlin.fadp.webapp.event.EventHandler;
import com.hanlin.fadp.webapp.event.EventHandlerException;
import com.hanlin.fadp.webapp.stats.ServerHitBin;
import com.hanlin.fadp.webapp.view.ViewFactory;
import com.hanlin.fadp.webapp.view.ViewHandler;
 ...
 * @Description: request处理句柄
 * @author: 祖国
 * @date: 2015年12月17日 下午10:13:50
 */
public class RequestHandler {

    public static final String module = RequestHandler.class.getName();
    /**requesthandler的默认状态码为302，在webapp/config/requestHandler.properties中配置状态码*/
    private final String defaultStatusCodeString = UtilProperties.getPropertyValue("requestHandler.properties", "status-code", "302");
    /**管理ViewHandler实例。通过获取controller.xml中handler的解析数据map， 该map中，
     * key=handler的name属性值，为view的类型，如ftl，value=handler中class属性值， 
     * 该属性值为ViewHandler接口的实现类，通过反射进行实例化，然后初始化，再放入到一个map中 */
    private final ViewFactory viewFactory;
    /**管理EventHandler实例数据。通过加载controller.xml中handler解析数据，其中handler类型为非view类型，
     *  如果类型为view类型，则调用ViewFactory，因此，本类的设计和ViewFactory设计是一样的 */
    private final EventFactory eventFactory;
    /** 从请求的上下文环境中获取的controller.xml文件所在的url */
     ...
     * @Description: 根据请求的上下文环境中的controller.xml文件的url，获取一个controller.xml解析类ControllerConfig实例
     * @return: ConfigXMLReader.ControllerConfig
     */
    ...
    /**
     * @Title: doRequest 
     * @Description: 处理请求的核心方法，从缓存中获取delegator和userlongin后，调用本来重载方法，
     * @param request 请求request对象
     * @param response 应答response对象
     * @param requestUri 请求uri字符串
    ...
    /**
     * @Title: doRequest 
     * @Description: 请求处理的核心方法，被ControlServlet调用
     * @param request
     * @param response
     * @param chain
     * @param userLogin
    ...
    public void doRequest(HttpServletRequest request, HttpServletResponse response, String chain,
        ...
        //获取请求上下文环境中controller.xml的解析类ControllerConfig实例，一次加载配置
        ...
        try {
        	//从请求的配置文件controller.xml文件解析类ControllerConfig实例中获取request-map解析的map
        ...
        //获取webapp的根名称，即项目应用名称
        String cname = UtilHttp.getApplicationName(request);

        //从request中获取pathInfo，再从pathInfo中获取第一个分隔符“/”前面部分，如果带参数，则去掉“？”以后部分，request中的pathInfo为servlet的path和请求变量之间的部分
        String defaultRequestUri = RequestHandler.getRequestUri(request.getPathInfo());
        //设置request中的targetRequestUri
        ...
        if (defaultRequestUri != null) {
        	//获取request-map元素对应的RequestMap实例。根据从request中获取的pathInfo中第一部分信息从解析controller.xml文件的request-map的map中获取
        	//相应的RequestMap实例，requestMapMap中key=request-map的uri属性值，value=request-map节点解析对于的RequestMap
            requestMap = requestMapMap.get(defaultRequestUri);//得到具体请求对应的RequestMap
        }
        // 如果上面的requestMap为空，则获取默认的请求配置，即default-request属性配置获取相应RequestMap，否则略去默认配置的查找
        if (requestMap == null) {
        ...
        // check for override view，需要再一次阅读
        ...
        //经过上面两种情况查找RequestMap，如果没有找到，则抛出异常
        if (requestMap == null) {
        ...
        if (requestMap.securityAuth) {
            ...
            //checkLogin配置在common-controller.xml中
        ...
        //处理_PREVIOUS_PARAM_MAP_FORM_存在，但_PREVIOUS_REQUEST_不存在情况
        ...
        //下面展开request-map节点中关于下一个responose子节点的处理
        ...
        if (eventReturn == null && requestMap.event != null) {
        	//如果event的type、path、invoke都非空的情况下执行下面的逻辑
            if (requestMap.event.type != null && requestMap.event.path != null && requestMap.event.invoke != null) {
                try {
                    ...
                    // 为请求事件记录服务命中纪录save the server hit for the request event
        ...
        //根据事件的处理结果eventReturn获取RequestResponse，
        ...
        if (eventReturnBasedRequestResponse != null) {
            ...
            //如果RequestResponse中name为error，则显示错误信息，如果为success，在后面进行处理
        ...
        if (previousRequest != null && loginPass != null && loginPass.equalsIgnoreCase("TRUE")) {
            ...
            if ("logout".equals(previousRequest) || "/logout".equals(previousRequest) || "login".equals(previousRequest) || "/login".equals(previousRequest) || "checkLogin".equals(previousRequest) || "/checkLogin".equals(previousRequest) || "/checkLogin/login".equals(previousRequest)) {
                ...
                //从request中通过_PREVIOUS_PARAM_MAP_URL_这个attribute获取相应的map，key为Stirng型，value为Object型
        ...
        //获取request-map节点的response子节点中name属性=success的子节点
        ConfigXMLReader.RequestResponse successResponse = requestMap.requestResponseMap.get("success");
        //如果event处理结果为null或结果为success，且response节点对应对象非空，且response节点对应对象的type属性
        //值为request，则将overrideViewUri设置到request的attribute中，key为_POST_CHAIN_VIEW_，将当前的response
        //子节点作为要处理的子节点
        ...
        //下面处理request-map节点下面response子节点的两种情况，一是type=request，另一种是type=view
        //如果type为request，则递归调用本方法，否则进入view-map的相关处理，直至结束
        if ("request".equals(nextRequestResponse.type)) {
        	...
        	//首先处理请求后的事件，即controller.xml和其include的相关XXX-controller.xml中配置的post-processor事件
            try {
            ...
            /**
             * 下面按request-map节点的response子节点中type属性的九种情况进行处理,前四种都调用callRedirect，
             * 其目的是改变request中attribute和session内容，以及设置response中的状态参数和头部参数
             * 5-8这四种调用renderView进行呈现工作，最后一种不做处理
             * 1、type为url：根据原请求的attribute，扩充session
             * 2、type为cross-redirect：
             * 3、type为request-redirect：处理重定向的配置
             * 4、type为request-redirect-noparam:
             * 5、type为view:
             * 6、type为view-last:
             * 7、type为view-last-noparam:
             * 8、type为view-home:
             * 9、type为none:
             */
    ...
    /**
     * @Title: getRequestUri 
     * @Description: 获取请求的uri，为请求path中第一个“/”分割的字符串，如果该字符串含“？”，则为“？”以前的字符串部分
     * @param path 请求的path
     * @return: String
     */
    ...
    /**
     * @Title: callRedirect 
     * @Description: 改变request的attribute、session设置，以及设置response的状态参数和头参数。
     * 根据原请求的attribute构建一个新的attribute，并将请求url放入到response中。
     * 具体做法是：获取原请求中的attribute，构建一个map，key=attribute的name属性，
     * 然后将该map转化为字节数组，将字节数组转化为16进制的字符串，放入到request的session中，
     * key=_REQ_ATTR_MAP_，value=转换后的16进制的字符串
     * @param url 请求的url
     * @param resp Response对象
     * @param req  Request对象
     * @param statusCodeString 状态码
     * @throws RequestHandlerException
     * @return: void
     */
    private void callRedirect(String url, HttpServletResponse resp, HttpServletRequest req, String statusCodeString) throws RequestHandlerException {
        ...
        //获取request的attribute的name属性
        Enumeration<String> attributeNameEnum = UtilGenerics.cast(req.getAttributeNames());
        //存放请求的attribute的map，key=attribute的name属性，value=attribute对象
        ...
        //遍历request中所有的attribute的name属性集，将其放入到一个map中，key=attribute的name，value=对于name的attribute对象
        ...
        //将上一步中得到的map中key为_REQUEST_HANDLER_情况剔除后序列化到一个字节数组中，然后放入到
        //request的session中，对应的attribute名为_REQ_ATTR_MAP_
     ...
     * @Title: renderView 
     * @Description: TODO
     * @param view 视图的name，一般为controller.xml中request-map节点的response子节点的value属性值
     * @param allowExtView 是否进行安全检查，一般为request-map节点的security子节点external-view属性值
     * @param req 请求Request对象
     * @param resp 应答Response对象
     * @param saveName view的name，一般为SAVED或HOME，如果response子节点的save-current-view属性非空，
     * 则为SAVED，如果response子节点的save-home-view属性非空，则为HOME
     * @throws RequestHandlerException
     * @return: void
     */
    private void renderView(String view, boolean allowExtView, HttpServletRequest req, HttpServletResponse resp, String saveName) throws RequestHandlerException {
        ...
        //获取servlet的path，并去掉最前面的“/”。注意获取的值和servlet的配置有关
        ...
        //下面代码相当于request.getPathInfo()，，即视图名中去掉“servlet/”部分后剩下部分
        ...
        //将上面得到的视图名放到request的attribute中，名为_CURRENT_VIEW_
     ...
     * @Description: 构建请求字符串。根据请求的重定向参数（controller.xml中request-map节点的response子节
     * 点的redirect-parameter孙子节点的配置解析信息）和请求Request对象，构建一个请求字符串。
     * 如果指定，则采用指定的参数，如果没有指定则采用request中的参数
     * @param request 请求Request对象
     * @param requestResponse controller.xml中request-map节点的response子节点
     * 的redirect-parameter孙子节点解析信息，即RequestResponse对象
     * @return: String
     */
    public String makeQueryString(HttpServletRequest request, ConfigXMLReader.RequestResponse requestResponse) {
    	//如果从controller.xml中获取的request-map节点的response子节点的解析类为空，或重定向参数为空且重定向值参数也为空，则对请求进行规范化处理
        if (requestResponse == null ||
            ...
            return "?" + queryString;
        } else {//controller.xml中获取的request-map节点的response子节点的解析类非空
            ...
            for (Map.Entry<String, String> entry: requestResponse.redirectParameterMap.entrySet()) {
                ...
                //用请求的name和value构成“？name=value”的形式放入StringBuffer中
    ...
    /**
     * @Title: addNameValuePairToQueryString 
     * @Description: 对请求的name、value对进行url编码
     * @param queryString 请求字符串
     * @param name 请求name字符串
     * @param value 请求name对应的value字符串
     * @return: void 返回一个字符串，其将name和value进行编码，形成name=value的形式附加到请求字符串后面
     */
    ...
    /**
     * @Title: makeLinkWithQueryString 
     * @Description: 构建一个url，一般为request-map节点的response子节点的value属性值，如main，前面加分隔符，最后
     * 形成的url如：/main
     * @param request Request对象
     * @param response Response对象
     * @param url 请求url，一般为："/" + request-map节点的response子节点的value属性值，如main
     * @param requestResponse request-map节点的response子节点的解析类
     * @return: String
     */
    ...
    /**
     * @Title: makeLink 
     * @Description: 对请求url进行编码处理，调用重构方法
     * @param request 请求Request
     * @param response 应答Response
     * @param url 请求url，带参数
     * @return: String
     */
    ...
    public static void main(String[] args){
    	...
    	List<String> pathItemList = StringUtil.split(path, "/");
    	pathItemList = pathItemList.subList(1, pathItemList.size());
    	System.out.println(pathItemList.toString());
    }
}
