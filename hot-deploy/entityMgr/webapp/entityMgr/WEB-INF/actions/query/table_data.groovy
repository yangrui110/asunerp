...
idelegator=DelegatorFactory.getDelegator(parameters.dataSourceName)
...
def eli = idelegator.find(entityName, condition, null, BaseInfo.getFieldSetWidthOutTimeStamp(idelegator.getModelEntity(entityName)), null, efo);
def totalRow=0;
def totalPages=0;
def record=0;
if(eli!=null)
...
def getTypeData(type, data, sdf){
	switch(type){
		...
		case "java.sql.Timestamp" : time = sdf.parse(data);println time;println time.getTime(); println new java.sql.Timestamp(time.getTime()); return new java.sql.Timestamp(time.getTime());
		default : return data;
	}
}
