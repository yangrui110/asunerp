...
under the License.
-->
<#assign appModelMenu = Static["com.hanlin.fadp.widget.model.MenuFactory"].getMenuFromLocation(applicationMenuLocation,applicationMenuName)>
<#if person?has_content>
  <#assign userName = person.partyName!>
...
<#if parameters.portalPageId?has_content && !appModelMenu.getModelMenuItemByName(headerItem)?? && userLogin??>
    <#assign findMap = Static["com.hanlin.fadp.base.util.UtilMisc"].toMap("portalPageId", parameters.portalPageId)>
...
<div id="control-area">
  <ul id="preferences-menu">
    <#if userLogin??>
      <#if (userPreferences.COMPACT_HEADER)?default("N") == "Y">
        <li class="collapsed"><a href="javascript:document.setUserPreferenceCompactHeaderN.submit()">&nbsp;</a>
          <form name="setUserPreferenceCompactHeaderN" method="post" action="<@fadpUrl>setUserPreference</@fadpUrl>">
      ...
      <#else>
        <li class="expanded"><a href="javascript:document.setUserPreferenceCompactHeaderY.submit()">&nbsp;</a>
          <form name="setUserPreferenceCompactHeaderY" method="post" action="<@fadpUrl>setUserPreference</@fadpUrl>">
            <input type="hidden" name="userPrefGroupTypeId" value="GLOBAL_PREFERENCES"/>
            <input type="hidden" name="userPrefTypeId" value="COMPACT_HEADER"/>
    ...
    <#if userLogin??>
      ...
      <li><a href="<@fadpUrl>logout</@fadpUrl>">${uiLabelMap.CommonLogout}</a></li>
      <li><a href="<@fadpUrl>ListVisualThemes</@fadpUrl>">${uiLabelMap.CommonVisualThemes}</a></li>
    <#else>
      <li><a href="<@fadpUrl>${checkLoginUrl}</@fadpUrl>">${uiLabelMap.CommonLogin}</a></li>
    </#if>
    <li class="first"><a href="<@fadpUrl>ListLocales</@fadpUrl>">${uiLabelMap.CommonLanguageTitle}</a></li>
    <#if userLogin??>
      <#if orgName?has_content>
        <li class="org">${orgName}</li>
      </#if>
     <#-- <#if userLogin.partyId??>
        <li class="user"><a href="<@fadpUrl>passwordChange</@fadpUrl>">${userName}</a></li>
      <#else> -->
